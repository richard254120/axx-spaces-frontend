...
// Injects button via pure vanilla JS directly into DOM
// Cannot be blocked by any React parent, overflow, or z-index issue
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      #wa-float-btn {
        ...
        animation: wa-ring 2s infinite !important;
        text-decoration: none !important;
      }
      #wa-float-btn:hover {
        transform: scale(1.14) !important;
        box-shadow: 0 8px 32px rgba(37,211,102,0.9) !important;
        animation: none !important;
      }
      @keyframes wa-ring {
        0%   { box-shadow: 0 0 0 0 rgba(37,211,102,0.7), 0 6px 24px rgba(37,211,102,0.5); }
        70%  { box-shadow: 0 0 0 14px rgba(37,211,102,0), 0 6px 24px rgba(37,211,102,0.5); }
    ...
    // --- BUTTON (anchor tag so it works even if JS click is blocked) ---
    const btn = document.createElement("a");
    btn.id = "wa-float-btn";
    btn.href = waUrl;
    btn.target = "_blank";
    btn.rel = "noopener noreferrer";
    btn.setAttribute("aria-label", "Chat with Axx Spaces on WhatsApp");
    btn.innerHTML = `
